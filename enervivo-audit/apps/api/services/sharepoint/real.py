...
class RealSharePointClient(SharePointClient):
    ...
    async def _resolve_folder(self, project_url: str) -> tuple[str, str, str]:
        ...
        # Garde-fou : on refuse tout path qui sort de la racine autorisée
        allowed = self._settings.sharepoint_allowed_root_path.rstrip("/")
        if allowed and not drive_path.startswith(allowed):
            raise PermissionError(
                f"Path '{drive_path}' hors de la racine autorisée '{allowed}'"
            )
